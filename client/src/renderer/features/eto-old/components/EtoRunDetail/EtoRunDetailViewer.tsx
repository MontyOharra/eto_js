...
import { useEtoRunDetail, useReprocessRun, useSkipRun } from "../../api/hooks";
import { EtoRunDetailHeader } from "./EtoRunDetailHeader";
...
interface EtoRunDetailViewerProps {
  isOpen: boolean;
  runId: number | null;
  onClose: () => void;
}
...
export function EtoRunDetailViewer({
  isOpen,
  runId,
  onClose,
}: EtoRunDetailViewerProps) {
  const { data: runDetail, isLoading, error: queryError } = useEtoRunDetail(
    isOpen ? runId : null
  );
  ...
  // Check if run has actionable status (failure or needs_template)
  // These statuses indicate there are sub-runs that can be reprocessed/skipped
  const hasActionableStatus = runDetail?.status === "failure" || runDetail?.status === "needs_template";
                    ...
                    <div className="flex-1 overflow-auto bg-gray-900 rounded p-3 relative">
                      {viewMode === "summary" ? (
                        runDetail.status === "success" ? (
                          <SummarySuccessView
                            executedActions={
                              runDetail.stage_pipeline_execution?.executed_actions || null
                            }
                          />
                        ...
                        <DetailPipelineView
                          pipelineDefinitionId={
                            runDetail.stage_pipeline_execution?.pipeline_definition_id || null
                          }
                          runDetail={runDetail}
                        />
                      )}
