...
export function TestingStep({
  ...
  useEffect(() => {
    ...
    const specificsContainer = document.querySelector('.testing-resize-container .testing-specifics-panel');
  ...
  return (
    <div className="pr-4 pl-2 py-4 flex h-full testing-resize-container">
      ...
      <div className="flex flex-col testing-specifics-panel" style={{ width: `${specificsWidth}%` }}>
        <div className="bg-gray-800 border border-gray-700 rounded-lg p-4 flex flex-col flex-1 overflow-hidden">
          <h3 className="text-lg font-semibold text-white mb-3">
            {viewMode === 'summary'
              ? (simulationResult.status === 'success' ? 'Actions Executed' : 'Error Details')
          ...
          <div className="flex-1 overflow-auto bg-gray-900 rounded p-3 relative">
            ...
            >
              {simulationResult.status === 'success' && simulationResult.pipeline_execution?.executed_actions ? (
                <pre className="text-gray-300 whitespace-pre-wrap break-words">
                  {JSON.stringify(simulationResult.pipeline_execution.executed_actions, null, 2)}
                </pre>
