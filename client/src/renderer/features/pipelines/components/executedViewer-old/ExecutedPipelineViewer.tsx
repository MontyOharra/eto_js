...
const createMinimalModuleTemplate = (
  moduleId: string,
  version: string,
  moduleName: string,
  color: string
...
export const ExecutedPipelineViewer = forwardRef<
  ExecutedPipelineViewerRef,
  ExecutedPipelineViewerProps
>(({ pipelineDefinitionId, executionData, extractedData }, ref) => {
  ...
  useEffect(() => {
    const loadPipeline = async () => {
      try {
        console.log(
          "[ExecutedPipelineViewer] Fetching pipeline:",
        ...
        console.log(
          "[ExecutedPipelineViewer] Executed node IDs:",
          Array.from(executedNodeIds)
        );
        console.log("[ExecutedPipelineViewer] Failed modules:", failedModules);
        console.log(
          "[ExecutedPipelineViewer] Execution values:",
          Array.from(executionValues.entries())
        );
        ...
        apiState.modules.forEach((module: any) => {
          if (!seenModuleRefs.has(module.module_ref)) {
            ...
            const actualModule = allModules.find(
              (m: ModuleTemplate) => m.id === moduleId
            );
            ...
            if (actualModule) {
              ...
              console.warn(
                `[ExecutedPipelineViewer] Module ${moduleId} not found in modules list, using fallback`
              );
              templates.push(
        ...
        const entryPoints: EntryPoint[] = apiState.entry_points.map(
          (ep: any) => {
        ...
        const modules: ModuleInstance[] = apiState.modules.map(
          (module: any) => {
            const converted = {
              ...
              inputs: module.inputs.map((input: any, idx: number) => ({
                ...
                allowed_types: Array.isArray(input.type)
                  ? input.type
                  : [input.type],
              })),
              outputs: module.outputs.map((output: any, idx: number) => ({
                ...
                allowed_types: Array.isArray(output.type)
                  ? output.type
                  : [output.type],
              })),
            };
            console.log(
              "[ExecutedPipelineViewer] Converted module:",
        ...
        const connections = apiState.connections
          .filter((conn: any) => {
            ...
            if (!shouldShow) {
              console.log(
                "[ExecutedPipelineViewer] Hiding connection:",
                conn.from_node_id,
                "→",
                conn.to_node_id,
        ...
        console.log(
          "[ExecutedPipelineViewer] Showing",
          connections.length,
          "of",
          apiState.connections.length,
          "connections"
        );
        ...
        console.log(
          "[ExecutedPipelineViewer] Auto-layout positions:",
          autoPositions
        );
        Object.keys(autoPositions).forEach((nodeId) => {
          const pos = autoPositions[nodeId];
          const isEntryPoint = apiState.entry_points.some(
            (ep: any) => ep.id === nodeId
          );

          console.log(
            "[ExecutedPipelineViewer] Positioning node:",
            nodeId,
            "isEntryPoint:",
            isEntryPoint,
            "pos:",
        ...
        console.log(
          "[ExecutedPipelineViewer] Final visual state:",
          convertedVisual
        );

        console.log(
          "[ExecutedPipelineViewer] Converted state:",
          convertedState
        );
        console.log(
          "[ExecutedPipelineViewer] Auto-layout positions:",
