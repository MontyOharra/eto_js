...
import { PipelineState, VisualState, EntryPoint } from "../../types";
...
export const ExecutedPipelineGraph = forwardRef<
...
const ExecutedPipelineGraphInner = forwardRef<
  ExecutedPipelineGraphRef,
  ExecutedPipelineGraphProps
>(
  (
    ...
    useEffect(() => {
      console.log(
        "[ExecutedPipelineGraph] Building nodes and edges from pipeline state"
      );
      ...
      pipelineState.modules.forEach((moduleInstance) => {
        ...
        if (!template) {
          console.error(
            `[ExecutedPipelineGraph] Template not found for module: ${moduleInstance.module_ref}`
          );
          console.error(
            `[ExecutedPipelineGraph] Looking for template ID: "${templateId}"`
          );
          console.error(
            `[ExecutedPipelineGraph] Available template IDs:`,
      ...
      const { nodes: layoutedNodes, edges: layoutedEdges } =
        getLayoutedElements(newNodes, edgesWithData);
