...
const PipelineGraphInner = forwardRef<PipelineGraphRef, PipelineGraphProps>(
  (
    ...
    // DISABLED: Dynamic entry point sync - now handled by mount/unmount pattern
    // Entry points are initialized once on mount by usePipelineInitialization
    // and persist in parent state between mounts
    /*
    // Track previous entry points to avoid infinite loops
    const prevEntryPointsRef = useRef<string>('');
    const hasTriggeredOnChangeForEntryPointsRef = useRef<string>('');

    // Sync entry points dynamically when they change (after initialization)
    useEffect(() => {
      // Serialize entry points to detect changes
      const entryPointsKey = entryPoints.map((ep) => ep.node_id).sort().join(',');

      // Skip if entry points haven't changed
      if (prevEntryPointsRef.current === entryPointsKey) {
        return;
      }

      prevEntryPointsRef.current = entryPointsKey;

      console.log('[PipelineGraph] Syncing entry points:', {
        count: entryPoints.length,
        ids: entryPointsKey,
      });

      setNodes((prevNodes) => {
        // Get current entry point node IDs
        const currentEntryPointIds = new Set(
          prevNodes
            .filter((n) => n.data.moduleInstance?.module_ref === 'entry_point:1.0.0')
            .map((n) => n.data.moduleInstance.outputs[0]?.node_id)
        );

        // Get new entry point IDs
        const newEntryPointIds = new Set(entryPoints.map((ep) => ep.node_id));

        // Check if there are differences
        const entriesAdded = entryPoints.filter((ep) => !currentEntryPointIds.has(ep.node_id));
        const entriesRemoved = Array.from(currentEntryPointIds).filter((id) => !newEntryPointIds.has(id));

        if (entriesAdded.length === 0 && entriesRemoved.length === 0) {
          return prevNodes; // No changes needed
        }

        console.log('[PipelineGraph] Updating nodes:', {
          added: entriesAdded.length,
          removed: entriesRemoved.length,
        });
        // Remove old entry point nodes
        let updatedNodes = prevNodes.filter((node) => {
          if (node.data.moduleInstance?.module_ref !== 'entry_point:1.0.0') return true;
          const nodeId = node.data.moduleInstance.outputs[0]?.node_id;
          return !entriesRemoved.includes(nodeId);
        });

        // Calculate position for new entry points based on visual state (deterministic)
        // Collect all positions from visual state
        const allPositions: { x: number; y: number }[] = [];

        // Add module positions from visual state
        if (initialVisualState?.modules) {
          Object.values(initialVisualState.modules).forEach(pos => {
            allPositions.push(pos);
          });
        }

        // Add existing entry point positions from visual state
        if (initialVisualState?.entryPoints) {
          Object.values(initialVisualState.entryPoints).forEach(pos => {
            allPositions.push(pos);
          });
        }

        // Entry points should be positioned to the LEFT of ALL elements (modules + existing entry points)
        let startX = 50;
        let startY = 50;

        if (allPositions.length > 0) {
          // Find leftmost element (could be module OR entry point) from visual state
          const leftmostX = Math.min(...allPositions.map((p) => p.x));

          // Find the Y coordinate of the leftmost element
          const leftmostElement = allPositions.find((p) => p.x === leftmostX);
          const leftmostY = leftmostElement ? leftmostElement.y : 50;

          // Position new entry points to the LEFT of the leftmost element
          // Place them 250px to the left
          startX = leftmostX - 250;
          startY = leftmostY;

          console.log('[PipelineGraph] Calculated entry point start position:', {
            startX,
            startY,
            leftmostX,
            leftmostY,
            totalElementsInVisualState: allPositions.length,
          });
        }

        // Count how many entry points already exist in visual state
        const existingEntryPointCount = initialVisualState?.entryPoints
          ? Object.keys(initialVisualState.entryPoints).length
          : 0;

        console.log('[PipelineGraph] Existing entry point count from visual state:', existingEntryPointCount);

        // Add new entry point nodes
        const newNodes = entriesAdded.map((ep, index) => {
          ...
          console.log('[PipelineGraph] Creating entry point node:', {
            name: ep.name,
            index,
            totalIndex: existingEntryPointCount + index,
        ...
        console.log('[PipelineGraph] Entry points synced. New nodes array includes:', {
          totalNodes: updatedNodes.length + newNodes.length,
          entryPointNodesAdded: newNodes.length,
        });

        return [...updatedNodes, ...newNodes];
      });
    }, [entryPoints, setNodes]);
    */
