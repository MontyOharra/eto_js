...
export const ConnectionInfoOverlay: React.FC<ConnectionInfoOverlayProps> = ({
  ...
  for (const module of modules) {
    // Get all outputs (handle both array and grouped format)
    const allOutputs = Array.isArray(module.outputs)
      ? module.outputs
      : [...(module.outputs.static || []), ...(module.outputs.dynamic || [])];

    // Get all inputs (handle both array and grouped format)
    const allInputs = Array.isArray(module.inputs)
      ? module.inputs
      : [...(module.inputs.static || []), ...(module.inputs.dynamic || [])];
    ...
    const inputNode = allInputs.find(n => n.node_id === connection.to_node_id);
    if (inputNode) {
      toModule = module;
      toNode = inputNode;
    }
