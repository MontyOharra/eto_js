...
export const Route = createFileRoute('/dashboard/eto-old/')({
  component: EtoPage,
});
...
function EtoPage() {
  ...
  const reprocessBulkMutation = useReprocessRuns();
  const skipBulkMutation = useSkipRuns();
  const deleteMutation = useDeleteRuns();
  // Single run operations (aggregates failed sub-runs)
  const reprocessRunMutation = useReprocessRun();
  const skipRunMutation = useSkipRun();
  ...
  const handleSkip = async (runId: number) => {
    try {
      await skipRunMutation.mutateAsync(runId);
  ...
  const handleReprocess = async (runId: number) => {
    try {
      await reprocessRunMutation.mutateAsync(runId);
      // TanStack Query auto-invalidates and refetches on success
    } catch (err) {
  ...
  const handleSaveTemplate = async (templateData: TemplateBuilderData) => {
    ...
    try {
      ...
      if (templateBuilderRunId) {
        ...
        await reprocessRunMutation.mutateAsync(templateBuilderRunId);
      ...
      <div className="space-y-4">
        ...
        <EtoRunTable
          ...
          onBulkSkip={async (runIds) => {
            await skipBulkMutation.mutateAsync({ run_ids: runIds });
          }}
          onBulkReprocess={async (runIds) => {
            await reprocessBulkMutation.mutateAsync({ run_ids: runIds });
          }}
        />
