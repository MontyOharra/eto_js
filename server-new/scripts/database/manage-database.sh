...
execute_database_operation() {
    ...
    # Execute the Python script - capture output to display on error
    local output
    local exit_code

    if [[ "$VERBOSE" == "true" ]]; then
        # In verbose mode, show all output in real-time
        ...
        # If there was an error, display the captured output
    ...
    if execute_database_operation "$ACTION"; then
        ...
        case $exit_code in
            2)
                ...
                log_error "Check the error details above for troubleshooting information"
            ...
            3)
                ...
                ;;
            *)
                log_error "Database operation failed (exit code: $exit_code)"
                log_error "Check the error details above for troubleshooting information"
                exit $EXIT_GENERAL_ERROR
                ;;
        esac
