...
Implementation using Windows COM interface with dataclass types and registry pattern
"""
import logging
import threading
import tempfile
...
@IntegrationRegistry.register(
    ...
    description="Connect to locally installed Outlook using COM interface",
...
class OutlookComIntegration(BaseEmailIntegration):
    """
    Outlook COM implementation using Windows COM interface.
    ...
    def __init__(self, email_address: Optional[str] = None, folder_name: str = "Inbox"):
        """
        Initialize Outlook COM integration

        Args:
            email_address: Email address to connect to (None = default account)
            folder_name: Default folder to monitor
        """
        super().__init__(email_address=email_address, folder_name=folder_name)
        ...
        self.logger.debug(f"Initialized OutlookComIntegration for {email_address or 'default account'}")
    ...
    def connect(self, account_identifier: Optional[str] = None) -> bool:
        ...
        try:
            with self._lock:
                # Use account from init if not provided
                if not account_identifier:
                    account_identifier = self.email_address
                ...
                if not self.namespace:
                    raise ConnectionError("Failed to get Outlook MAPI namespace")
                ...
                self.current_folder = self._get_folder_by_name(self.folder_name)
                if not self.current_folder:
                    raise ConnectionError(f"Folder '{self.folder_name}' not found")

                # Test folder access
                try:
                    folder_count = self.current_folder.Items.Count
                    self.logger.debug(f"Folder '{self.folder_name}' has {folder_count} items")
                except Exception as e:
                    raise ConnectionError(f"Cannot access folder '{self.folder_name}': {e}")
    ...
    def get_recent_emails(
        ...
        """Get recent emails from specified folder with cached attachments"""
        emails = []
        ...
        try:
            ...
            if folder_name != self.folder_name or not self.current_folder:
            ...
            try:
                ...
                # Convert to EmailMessage dataclasses AND cache attachments
                count = 0
                for mail_item in items:
                    ...
                    try:
                        ...
                        # PERFORMANCE: Cache attachments now while we have the mail_item
                        if hasattr(mail_item, 'Attachments') and mail_item.Attachments.Count > 0:
                            attachment_start = time.time()
                            self.logger.debug(
                                f"Caching {mail_item.Attachments.Count} attachments "
                                f"for email {email_msg.message_id[:20]}..."
                            )
                            ...
                            self.logger.info(
                                f"Cached {len(email_msg.cached_attachments)} attachments "
                                f"in {attachment_duration:.3f}s"
                            )
    ...
    def get_attachments(self, message_id: str, folder_name: str = "Inbox") -> list[EmailAttachment]:
        ...
        try:
            self.logger.info(f"get_attachments() STARTED for {message_id[:20]}...")
            ...
            try:
                ...
                for i, attachment in enumerate(mail_item.Attachments):
                    try:
                        ...
                        self.logger.warning(f"Failed to extract attachment {i+1}/{attachment_count}: {e}")
                        continue

                extraction_duration = time.time() - extraction_start
                total_duration = time.time() - method_start_time
                self.logger.info(
                    f"get_attachments() COMPLETED: {len(attachments)} attachments "
    ...
    def _test_connection_internal(self) -> ConnectionTestResult:
        ...
        try:
            ...
            try:
                ...
                # Test folder access
                test_folder = self._get_folder_by_name_test(test_namespace, self.folder_name)
                if not test_folder:
                    return ConnectionTestResult(
                        success=False,
                        message=f"Folder '{self.folder_name}' not accessible",
                        error=f"Cannot find folder: {self.folder_name}"
                    )
    ...
    def _get_folder_by_name_threadsafe(self, namespace, folder_name: str):
    ...
    def _get_folder_by_name_test(self, namespace, folder_name: str):
    ...
    def _search_folder_recursive(self, parent_folder, target_name: str, max_depth: int = 3, current_depth: int = 0):
    ...
    def _discover_folders_recursive(
        self,
        parent_folder,
        folder_list: list[EmailFolder],
        current_depth: int = 0,
        max_depth: int = 3,
        parent_path: str = ""
    ):
    ...
    def _convert_to_email_message(self, outlook_mail, folder_name: str) -> EmailMessage:
        """Convert Outlook mail item to EmailMessage dataclass"""
        try:
    ...
    def _convert_outlook_time_to_utc(self, outlook_time) -> datetime:
        """Convert Outlook time to UTC datetime"""
        try:
            # Outlook returns local time
            ...
            self.logger.warning(f"Error converting time, using current UTC: {e}")
            return datetime.now(timezone.utc)

    def _extract_attachment(self, attachment) -> Optional[EmailAttachment]:
        """Extract attachment and convert to EmailAttachment dataclass"""
        temp_path = None
