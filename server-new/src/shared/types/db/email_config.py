...
class EmailConfig(EmailConfigBase):
    ...
    @classmethod
    def from_db_model(cls, db_model : EmailConfigModel) -> 'EmailConfig':
...
class EmailConfigSummary(BaseModel):
    ...
    @classmethod
    def from_full_config(cls, config: EmailConfig) -> 'EmailConfigSummary':
        """Create summary from full email config model"""
        return cls(
            id=config.id,
            name=config.name,
            ...
            filter_rule_count=len(config.filter_rules) or 0, 
