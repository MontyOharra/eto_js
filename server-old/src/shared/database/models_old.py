...
class PdfFileModel(BaseModel):
    ...
    eto_runs: Mapped['EtoRunModel']          = relationship(back_populates="pdf_file")
    pdf_template: Mapped['PdfTemplateModel'] = relationship(back_populates="pdf_file")
...
class PdfTemplateModel(BaseModel):
    ...
    # Relationships
    pdf: Mapped['PdfFileModel']                                    = relationship(back_populates="pdf_template")
    pdf_template_versions: Mapped[List['PdfTemplateVersionModel']] = relationship(back_populates="pdf_template")
...
class PdfTemplateVersionModel(BaseModel):
    ...
    # Relationships
    pdf_template : Mapped['PdfTemplateModel'] = relationship(back_populates="pdf_template_versions")

class PipelineCompiledPlanModel(BaseModel):
    """
    Table for the compiled plan of a pipeline
    """
    ...
    compiled_at: Mapped[datetime] = mapped_column(DateTime, nullable=False)
...
class PipelineDefinitionStepModel(BaseModel):
    ...
    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    pipeline_compiled_plan_id: Mapped[int] = mapped_column(Integer, ForeignKey("pipeline_compiled_plans.id"), nullable=False)
    ...
    compiled_plan = relationship("PipelineCompiledPlanModel", back_populates="steps")
