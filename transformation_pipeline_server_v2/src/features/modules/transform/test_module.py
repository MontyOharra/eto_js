...
class StringConcatenator(TransformModule):
                ...
                inputs=IOSideShape(
                    static= StaticNodes(
                        slots=[
                            ...
                            NodeSpec(
                                label="test1",
                                typing=NodeTypeRule(allowed_types=[])  # Multiple allowed types
                            ),
                            NodeSpec(
                                label="test2",
                                typing=NodeTypeRule(allowed_types=["bool"])  # Multiple allowed types
                            ),
                            NodeSpec(
                                label="test3",
                    ...
                    dynamic=DynamicNodes(
                        groups=[
                            ...
                            DynamicNodeGroup(
                                min_count=1,  # At least two inputs required
                                max_count=3,  # Unlimited inputs
                                item=NodeSpec(
                                    label="d1",
                                    typing=NodeTypeRule(type_var="T")  # Multiple allowed types
                                )
                ...
                outputs=IOSideShape(
                    static=StaticNodes(
                        slots=[
                            ...
                            NodeSpec(
                                label="out1",
                                typing=NodeTypeRule(allowed_types=["datetime", "float"])  # Multiple allowed types
                            ),
                            NodeSpec(
                                label="out2",
                                typing=NodeTypeRule(type_var="U")  # Multiple allowed types
    ...
    def run(self, inputs: Dict[str, Any], cfg: TestConfig, context: Any = None) -> Dict[str, Any]:
        """
