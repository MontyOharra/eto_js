...
# ETO Transformation Pipeline Server Scripts Convenience Script (Bash)
...
show_help() {
    echo -e "${CYAN}ETO Transformation Pipeline Server Scripts${NC}"
    echo -e "${CYAN}===========================================${NC}"
    echo ""
    echo -e "Usage: ./server-scripts.sh <action>${NC}"
    echo ""
    echo -e "${YELLOW}Actions:${NC}"
    echo -e "  build      - Build and deploy Flask server to C:\\apps\\eto\\transformation_pipeline_server${NC}"
...
case "$ACTION" in
    "build")
        echo -e "${GREEN}Building and deploying Transformation Pipeline server...${NC}"
        "$SCRIPTS_DIR/build-server.sh"
        ;;
    "refresh")
        echo -e "${GREEN}Refreshing deployed Transformation Pipeline server files (no venv rebuild)...${NC}"
        "$SCRIPTS_DIR/refresh-server.sh"
        ;;
    "start")
        echo -e "${GREEN}Starting Transformation Pipeline server...${NC}"
        "$SCRIPTS_DIR/server-start.sh"
        ;;
    "kill")
        echo -e "${GREEN}Killing Transformation Pipeline server instances...${NC}"
    ...
    "killpython")
        ...
        ;;
    "resetdb")
        echo -e "${GREEN}Resetting transformation pipeline database (dropping and recreating all tables)...${NC}"
