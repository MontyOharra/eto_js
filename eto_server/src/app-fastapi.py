...
async def initialize_services() -> None:
    ...
    try:
        ...
        _service_container = _get_container()
...
async def cleanup_services() -> None:
    ...
    try:
        if _service_container:
            # Add any service cleanup logic here if needed
...
def register_routers(app: FastAPI) -> None:
    """Register all FastAPI routers from the feature-based API structure"""
    try:
        # Import and register FastAPI routers
        from .api.routers import ROUTERS

        for router in ROUTERS:
            app.include_router(router)
            logger.debug(f"Registered router: {router.tags}")

        logger.info(f"Successfully registered {len(ROUTERS)} routers")

        # TODO: Convert remaining Flask blueprints to FastAPI routers:
        # - health_bp -> health_router
        # - email_ingestion_bp -> email_ingestion_router
        # - eto_processing_bp -> eto_processing_router
        # - eto_service_bp -> eto_service_router
        # - pdf_viewing_bp -> pdf_viewing_router
...
def create_app() -> FastAPI:
    ...
    Creates and configures the FastAPI app with all routers and services
    """
