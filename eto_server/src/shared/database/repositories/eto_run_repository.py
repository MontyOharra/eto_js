...
class EtoRunRepository(BaseRepository[EtoRunModel]):
    ...
    def _convert_to_domain_object(self, eto_run_model: EtoRunModel) -> "EtoRun":
        ...
        eto_run_data = {
            ...
            'template_match_coverage': getattr(eto_run_model, 'template_match_coverage'),
            ...
            'failed_step_id': getattr(eto_run_model, 'failed_step_id'),
        ...
        # Import EtoRun dynamically to avoid circular imports
        ...
        try:
            with self.connection_manager.session_scope() as session:
                ...
                if model:
                    ...
                    logger.debug(f"Retrieved ETO run: {getattr(model, 'id')} for email {getattr(model, 'email_id')}")
    ...
    def get_by_pdf_id(self, pdf_file_id: int) -> List["EtoRun"]:
        ...
        try:
            with self.connection_manager.session_scope() as session:
                models = session.query(self.model_class).filter(
                    self.model_class.pdf_file_id == pdf_file_id
                ).all()
    ...
    def get_by_template_id(self, template_id: int) -> List["EtoRun"]:
        ...
        try:
            with self.connection_manager.session_scope() as session:
                models = session.query(self.model_class).filter(
                    self.model_class.matched_template_id == template_id
                ).all()
    ...
    def update_processing_step(self, run_id: int, status: str, processing_step: str, **kwargs) -> Optional["EtoRun"]:
    ...
    def mark_as_failed(self, run_id: int, error_message: str, error_type: str = None, error_details: Dict[str, Any] = None) -> Optional["EtoRun"]:
    ...
    def get_runs_by_status(self, status_list: List[str], limit: Optional[int] = None) -> List["EtoRun"]:
